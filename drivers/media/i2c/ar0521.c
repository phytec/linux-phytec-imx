...
static int ar0521_vv_set_sensormode(struct ar0521 *sensor, void *args)
{
	...
	sel.r.top = (AR0521_DEF_HEIGHT - ar0521_modes[index].height) / 2 + 4;
	sel.r.left = (AR0521_DEF_WIDTH - ar0521_modes[index].width) / 2 + 4;
	sel.r.width = ar0521_modes[index].width;
	sel.r.height = ar0521_modes[index].height;
	...
	ret = ar0521_set_selection(sd, &cfg, &sel);
	if (ret)
		return ret;

	ret = ar0521_set_fmt(sd, &cfg, &format);
	if (ret)
		return ret;
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fmt = ar0521_get_pad_fmt(sensor, cfg, fse->pad, fse->which);
	crop = ar0521_get_pad_crop(sensor, cfg, fse->pad, fse->which);
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	fmt = ar0521_get_pad_fmt(sensor, cfg, format->pad, format->which);
	crop = ar0521_get_pad_crop(sensor, cfg, format->pad,
				...
				struct v4l2_subdev_selection *sel)
{
	...
	_crop = ar0521_get_pad_crop(sensor, cfg, sel->pad, sel->which);

...
#ifdef I2C_DEBUG
	dev_err(&client->dev, "WRITE len: %u", wr_len);
	for (j = 0; j < wr_len; j++) {
		dev_err(&client->dev, "DATA[%d] = %x", j, wr_buf[j]);
	}
#endif
...
#ifdef I2C_DEBUG
	dev_err(&client->dev, "READ len: %u\n", rd_len);
	for (j = 0; j < rd_len; j++) {
		dev_err(&client->dev, "DATA[%d] = %x", j, rd_buf[j]);
	}
#endif
